...
//   GET  /program/:id/feed               -> every linked athlete's workouts + week reviews,
...
async function programFeedData(env, programId, viewerAthleteId) {
  ...
  const wrapups = await env.DB.prepare(
    'SELECT athlete_id AS athleteId, week_id AS weekId, data, created_at AS createdAt, updated_at AS updatedAt FROM week_wrapups WHERE program_id = ?'
  ).bind(programId).all();
...
async function handleWrapupList(request, env, cors, programId) {
  ...
  const rows = await env.DB.prepare(
    'SELECT week_id AS weekId, created_at AS createdAt, COALESCE(updated_at, created_at) AS updatedAt FROM week_wrapups WHERE program_id = ? AND athlete_id = ?'
...
async function handleWrapupPost(request, env, cors, programId, weekId) {
  ...
  return json({ weekId: w.id, createdAt: row.createdAt, updatedAt: row.updatedAt, summary }, 200, cors);
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (seg[0] === 'program' && seg[2] === 'wrapups' && !seg[3] && method === 'GET') return handleWrapupList(request, env, cors, seg[1]);
      if (seg[0] === 'program' && seg[2] === 'wrapups' && seg[3] && !seg[4] && method === 'POST') return handleWrapupPost(request, env, cors, seg[1], seg[3]);
